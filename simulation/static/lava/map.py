...
#-----------------------------MAIN PROGRAM---------------------------------#
#--------------------------------------------------------------------------#
def main(volume, viskositas):
	print("INITIALIZING")
	print(volume, viskositas)

	diy = Image.open('simulation\Static\lava\DIY.tif')		#buka DEM
	diy_arr = np.array(diy)						#ubah DEM jadi array
	kawah = getKawah(diy_arr)					#data koordinat kawah

	#buka map buat ditampilin
	base_map = Image.open('simulation\Static\lava\DIY_render.tif')
	...
	base_map.save("simulation/static/lava/DIY_result.jpg", 'JPEG')
